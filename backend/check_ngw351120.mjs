...
async function checkDesign() {
  try {
    ...
    if (customerReturns.length > 0) {
      console.log('🟢 CUSTOMER RETURN DETAILS (Items returned by customers):');
      customerReturns.slice(0, 10).forEach((r, i) => {
        console.log(`   ${i+1}. Color: ${r.color}, Size: ${r.size}, Qty: ${r.qty}, Channel: ${r.channel || 'retail'}, Date: ${new Date(r.date).toISOString().split('T')[0]}`);
      });
      if (customerReturns.length > 10) {
        console.log(`   ... and ${customerReturns.length - 10} more`);
      }
      console.log('');
    }
