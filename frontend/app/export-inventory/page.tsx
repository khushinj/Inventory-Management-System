...
export default function ExportInventoryPage() {
  ...
  const fetchInventory = async () => {
    try {
      ...
      // Fetch summary
      const summaryRes = await api.get(`/inventory/warehouse/export/summary`);
      setSummary(summaryRes.data);
      setItems(summaryRes.data.items || []);

      // Fetch low stock items
      const lowStockRes = await api.get(
        `/inventory/warehouse/export/low-stock?threshold=${threshold}`
      );
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="bg-gradient-to-r from-emerald-600 to-teal-600 text-white shadow-lg">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
          <h1 className="text-3xl font-bold">Export Warehouse Inventory</h1>
          <p className="mt-1 text-emerald-100">View and manage export warehouse stock levels</p>
        </div>
      </div>
