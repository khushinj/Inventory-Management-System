...
export default function EditExportInventoryPage() {
  ...
  const fetchData = async () => {
    try {
      ...
      const inventoryResponse = await api.get("/inventory/warehouse/export");
      ...
      <div className="bg-white border-b shadow-sm">
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
          <div className="flex items-center justify-between">
            <div>
              <h1 className="text-2xl font-bold text-gray-900">Edit Product Image - Export</h1>
              <p className="text-sm text-gray-500 mt-1">Design Number: {designNumber}</p>
            </div>
            <button onClick={() => router.push("/export-inventory")} className="px-4 py-2 text-gray-600 hover:text-gray-800 font-medium">
              ← Back to Inventory
            </button>
          </div>
        </div>
          ...
          <div className="lg:col-span-2">
            <form onSubmit={handleSubmit} className="bg-white rounded-lg shadow p-6 space-y-6">
              ...
              <div className="flex gap-4 pt-4">
                ...
                <button type="button" onClick={() => router.push("/export-inventory")} className="px-6 py-3 text-black border border-gray-300 rounded-lg font-semibold hover:bg-gray-50 transition-colors">
                  Cancel
                </button>
              </div>
            </form>
