...
function ExportDashboard() {
  ...
  useEffect(() => {
    ...
    const validFormTypes = ["dispatch", "production", "purchase", "transfer"];
  ...
  const handleSaveAndContinue = async (entryId?: string) => {
    try {
      ...
      if (entryId) {
        await api.patch(`/warehouse/export/${entryId}`, payload);
        setEditingEntry(null);
      } else {
        await api.post("/warehouse/export", payload);
      }
  ...
  const handleExportToExcel = () => {
    ...
    XLSX.utils.book_append_sheet(workbook, worksheet, "Export Transactions");
    XLSX.writeFile(workbook, `export_transactions_${selectedFormType}_${new Date().toISOString().split("T")[0]}.xlsx`);
  };
    ...
    reader.onload = async (event) => {
      try {
        ...
        for (const row of jsonData) {
          await api.post("/warehouse/export", {
            dno: row.DNO || row.dno,
        ...
        <div className="bg-white shadow rounded-lg p-6 mb-6">
          ...
          <div className="mb-6 flex gap-3">
            <button
              ...
              className="bg-violet-600 text-white px-4 py-2 rounded-lg font-medium hover:bg-violet-700 transition-colors flex items-center gap-2"
            >
          ...
          <div className="mb-6 border-b pb-4">
            ...
            {lockedFormType ? (
              <div className="bg-purple-50 border border-purple-300 px-6 py-3 rounded-lg">
                <p className="text-gray-600">Form Type: <span className="font-semibold capitalize text-purple-700">{lockedFormType}</span></p>
              </div>
            ) : (
                                ...
                                <div className="absolute z-10 w-full mt-1 bg-white border border-gray-300 rounded shadow-lg max-h-48 overflow-y-auto">
                                  {filteredTransferOptions.map((option, idx) => (
                                    <div
                                      ...
                                      className="px-3 py-2 hover:bg-purple-50 cursor-pointer text-black"
                                    >
                                      {option}
                                    </div>
                                  ))}
                          ...
                          <td className="px-6 py-4">
                            <div className="relative">
                              <input
                                ...
                                className="w-full px-2 py-1 border rounded text-black bg-white"
                              />
                              {showChannelDropdown && filteredChannelOptions.length > 0 && (
                                <div className="absolute z-10 w-full mt-1 bg-white border border-gray-300 rounded shadow-lg max-h-48 overflow-y-auto">
                                  {filteredChannelOptions.map((option, idx) => (
                                    <div
                                      key={idx}
                                      onClick={() => {
                  ...
                  {filteredEntries.map((entry) => (
                    <tr key={entry._id}>
                      {editingEntry === entry._id ? (
                        <>
                          <td className="px-6 py-4"><input ref={dnoRef} type="text" value={editForm.dno} onChange={(e) => setEditForm({...editForm, dno: e.target.value})} onKeyDown={(e) => handleKeyDown(e, typeRef)} className="w-full px-2 py-1 border rounded text-black bg-white" /></td>
                          ...
                          {editForm.formType === "dispatch" && (
                            <td className="px-6 py-4"><input ref={additionalFieldRef} type="text" value={editForm.receiver} onChange={(e) => setEditForm({...editForm, receiver: e.target.value})} onKeyDown={(e) => handleKeyDown(e, undefined, true, entry._id)} placeholder="Receiver" className="w-full px-2 py-1 border rounded text-black bg-white" /></td>
                          )}
                          {editForm.formType === "purchase" && (
                            <td className="px-6 py-4"><input ref={additionalFieldRef} type="text" value={editForm.supplier} onChange={(e) => setEditForm({...editForm, supplier: e.target.value})} onKeyDown={(e) => handleKeyDown(e, undefined, true, entry._id)} placeholder="Supplier" className="w-full px-2 py-1 border rounded text-black bg-white" /></td>
                          )}
